...
const equipmentSlots: InventorySlotDefinition[] = Array.from(
  { length: 12 },
  (_, slotId) => ({ viewId: 'equipment', slotId }),
);
const beltSlots: InventorySlotDefinition[] = Array.from(
  { length: 6 },
  (_, index) => ({ viewId: 'belt', slotId: index + 12 }),
);
const slots = [...equipmentSlots, ...beltSlots];
</script>

<template>
  <section class="inventory" aria-label="Equipment and inventory slots">
    <InventorySlot v-for="slot in slots" :key="inventorySlotKey(slot)"
                   :class="`${slot.viewId}-${slot.slotId}`" :slot="slot" />
  </section>
</template>
