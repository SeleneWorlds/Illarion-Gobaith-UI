...
const open = () => {
  if (Math.round(minimap.cameraCoordinate.value.z) === 0) visible.value = true;
};
...
watch(minimap.revision, () => { if (visible.value) draw(); });
watch(minimap.cameraCoordinate, coordinate => {
  if (visible.value && Math.round(coordinate.z) !== 0) close();
});
...
<style scoped>
.world-map { position: absolute; z-index: 15; left: 212px; bottom: 134px; width: 600px; height: 500px; pointer-events: auto; }
.frame { position: absolute; inset: 0; width: 100%; height: 100%; filter: drop-shadow(0 4px 9px #000); pointer-events: none; }
.viewport { position: absolute; top: 50px; left: 50px; width: 500px; height: 400px; overflow: hidden; background: #000; }
.surface { display: block; width: 512px; height: 512px; image-rendering: pixelated; }
.crosshair, .crosshair::after { position: absolute; width: 20px; height: 1px; background: rgb(0 0 128); content: ''; pointer-events: none; transform: translate(-50%, -50%); }
.crosshair { top: 0; left: 0; }
.crosshair::after { top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(90deg); }
.close { position: absolute; left: 400px; bottom: 40px; width: 40px; height: 95px; padding: 0; border: 0; background: transparent; cursor: pointer; }
.close img { display: block; width: 100%; height: 100%; }
</style>
